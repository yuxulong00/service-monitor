...
def get_linkedin_url(url, s):
    ...
    try:
        ...
        if r.status_code == 302 and 'Location' in list(r.headers.keys()) and 'linkedin.com/in/' in r.headers[
            'Location']:
...
def parse(content, url):
    ...
    if firstname and lastname:
        ...
        if credit_grade and credit_grade[0] in list(CREDIT_GRADE.keys()):
            credit_lastModifiedAt_date = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(
                int(credit_lastModifiedAt[0][:10]))) if credit_lastModifiedAt else ''
            print('芝麻信用: %s %s' % (CREDIT_GRADE[credit_grade[0]],
                                   '   最后更新时间: %s' % credit_lastModifiedAt_date if credit_lastModifiedAt_date else ''))
        ...
        if wechat_name:
            print('微信昵称: %s %s' % (wechat_name[0],
                                   '    二维码(链接): %s' % wechat_image[0].replace('&#61;', '=').replace('&amp;',
...
def crawl(url, s):
    ...
    try:
        url = get_linkedin_url(url, copy.deepcopy(s)).replace('cn.linkedin.com',
                                                              'www.linkedin.com')  # 百度搜索出的结果是百度跳转链接，要提取出linkedin的链接。
...
if __name__ == '__main__':
    ...
    s = login(laccount='yuxulong22@gmail.com', lpassword='65217562')
    ...
    while len(url) > 0 and failure < 10:
        try:
            ...
            continue
        if r.status_code == 200:
            hrefs = list(set(re.findall('"(http://www\.baidu\.com/link\?url=.*?)"',
                                        r.content.decode(encoding='UTF-8', errors='strict'))))  # 一页有10个搜索结果
            for href in hrefs:
